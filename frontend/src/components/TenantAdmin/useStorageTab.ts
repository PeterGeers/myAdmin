/**
 * useStorageTab - logic hook for the Storage configuration tab.
 *
 * Extracted from StorageTab.tsx (pure refactor, no behavior change): holds all
 * data-loading callbacks, save handlers, and upload/OAuth flows so the component
 * itself is a thin render layer. State shapes and handler behavior are unchanged.
 */
...
export function useStorageTab(tenant: string) {
