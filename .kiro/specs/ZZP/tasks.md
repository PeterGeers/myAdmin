...
- [x] 17.1.5 Push to `feature/zzp-module`

### 17.2 Merge to Main

- [x] 17.2.1 Ensure `feature/zzp-module` is up to date with `main` (rebase or merge main into feature)
- [x] 17.2.2 Create PR from `feature/zzp-module` → `main`
- [x] 17.2.3 Merge PR
...
- [x] 17.3.1 Run `backend/sql/phase_zzp_tables.sql` — creates 7 tables (contacts, contact_emails, products, invoices, invoice_lines, invoice_number_sequences, time_entries), 1 view (vw_invoice_vat_summary), and seeds ZZP module for InterimManagement tenant
- [x] 17.3.2 Run `backend/sql/phase_zzp_revenue_account.sql` — adds `revenue_account` column to `invoices` table
- [x] 17.3.3 Run `backend/sql/phase_zzp_revenue_ledger_rename.sql` — migrates `zzp_invoice_ledger` flags on revenue accounts (8xxx) to `zzp_revenue_ledger`, cleans up incorrect flags
- [x] 17.3.4 Run `backend/sql/phase_zzp_branding_migration.sql` — copies `branding` namespace params to `zzp_branding` and `str_branding` namespaces (safe to rerun: uses INSERT IGNORE)
...
- [x] 17.4.1 Verify all 7 tables + 1 view exist: `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME IN ('contacts','contact_emails','products','invoices','invoice_lines','invoice_number_sequences','time_entries')`
- [x] 17.4.2 Verify `revenue_account` column on invoices: `SHOW COLUMNS FROM invoices LIKE 'revenue_account'`
- [x] 17.4.3 Verify ZZP module seed: `SELECT * FROM tenant_modules WHERE module_name = 'ZZP'`
...
- [x] 17.4.5 Verify ledger parameters: `SELECT Account, parameters FROM rekeningschema WHERE JSON_EXTRACT(parameters, '$.zzp_revenue_ledger') = true`

### 17.5 Production Smoke Test

- [x] 17.5.1 Smoke test: ZZP module visible in navigation when enabled
- [x] 17.5.2 Smoke test: Create contact → create product → create invoice → send flow
- [x] 17.5.3 Smoke test: ZZP docs accessible at `/docs/zzp/` (NL) and `/docs/en/zzp/` (EN)
- [x] 17.5.4 Smoke test: Onboarding page accessible at `/docs/getting-started/onboarding/`
- [x] 17.5.5 Smoke test: Existing STR invoice generation still works (branding namespace rename)

### 18.1 Documentation

- [x] Update spec `.kiro/specs/zzp-module/README.md` with final status and change log
