...
class TransactionLogic:
    ...
    def get_last_transactions(self, transaction_number, administration=None):
        ...
        if len(results) == 1:
            ...
            single_transaction_vendors = [
                "SomeVendor"
            ]  # Add vendors that use single transactions
    ...
    def prepare_new_transactions(self, template_transactions, new_data):
        ...
        for i, template in enumerate(template_transactions):
            ...
            if vendor_data:
                if i == 0:  # First transaction - main amount
                    amount = vendor_data.get(
                        "total_amount", template.get("TransactionAmount", 0)
                    )
                    description = vendor_data.get(
                        "description",
                        new_data.get(
                            "description", template.get("TransactionDescription", "")
                        ),
                    )
            ...
            if vendor_data:
                ...
                if (
                    "invoice_number" in vendor_data
                    and vendor_data["invoice_number"]
                    and new_data["folder_name"].lower() != "amazon"
                ):
    ...
    def save_approved_transactions(self, transactions):
        ...
        with self.db.transaction() as (cursor, conn):
            for transaction in transactions:
                ...
                if amount == 0:
                    print(
                        f"Skipping zero-amount transaction: {transaction.get('TransactionDescription', 'Unknown')} "
                        f"(Debet: {transaction.get('Debet')}, Credit: {transaction.get('Credit')})",
                        flush=True,
                    )
                    skipped_count += 1
                    continue
