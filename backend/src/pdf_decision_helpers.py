        ...
        if decision not in ["continue", "cancel"]:
            return {
                ...
                "error_response": create_error_response(
                    ...
                    errors=[
                        f'Decision must be "continue" or "cancel", got: {decision}'
                    ],
                    user_message="Invalid action selected. Please choose Continue or Cancel.",
                ),
            }
            ...
            if existing_transactions and new_file_url:
                ...
                try:
                    ...
                    if should_cleanup:
                        try:
                            cleanup_success = (
                                file_cleanup_manager.cleanup_uploaded_file(
    ...
    for i, transaction in enumerate(transactions):
        ...
        if missing_fields:
            validation_errors.append(
                f"Transaction {i + 1} missing fields: {missing_fields}"
            )
        ...
        try:
            if "amount" in transaction:
                ...
                if amount <= 0:
                    validation_errors.append(
                        f"Transaction {i + 1} has invalid amount: {amount}"
                    )
