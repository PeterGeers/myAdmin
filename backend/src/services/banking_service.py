        ...
        try:
            ...
            if matching_account:
                ...
                if iban_tenant != tenant:
                    ...
                    return {"valid": True, "tenant": iban_tenant}
            else:
                # IBAN not found in lookup - might be okay for new accounts
                print(
                    f"Warning: IBAN {iban} not found in bank account lookups",
                    flush=True,
                )
                return {
            ...
            if "Ref1" in df.columns:
                ...
                for iban in ibans:
                    ...
                    if not validation["valid"]:
                        ...
                        print(
                            "[TENANT VALIDATION] ALLOWED: IBAN matches current tenant",
                            flush=True,
                        )
            ...
            if use_enhanced:
                ...
                for pattern in patterns_data:
                    ...
                    if debet_val and str(debet_val) < "1300":
                        key = (
                            f"{pattern.get('administration')}_{debet_val}_{credit_val}"
                        )
                    ...
                    if not transaction.get("Credit"):
                        for pattern_group in debet_patterns.values():
                            if pattern_group["patterns"]:
                                ...
                                try:
                                    match = re.search(
                                        pattern_regex, description, re.IGNORECASE
                                    )
                                    if match:
    ...
    def save_transactions(
        ...
        try:
            ...
            for iban in ibans:
                ...
                existing_sequences = db.get_existing_sequences(
                    iban, table_name, administration=tenant
                )
                ...
                new_transactions = [
                    t
                    for t in iban_transactions
                    if t.get("Ref2") not in existing_sequences
                ]
            ...
            if saved_count > 0:
                ...
                invalidate_cache()
                print(
                    f"[CACHE] Invalidated cache after saving {saved_count} transactions",
                    flush=True,
                )
    ...
    def get_lookups(self, tenant: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_transactions = db.get_recent_transactions(
                limit=100, administration=tenant
            )
    ...
    def check_accounts(
        ...
        try:
            ...
            balances = processor.check_banking_accounts(
                end_date=end_date, administration=tenant
            )
