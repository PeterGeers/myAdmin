    ...
    def _load_from_google_drive(
        ...
        try:
            ...
            result = db.execute_query(
                query, [self.administration, template_name], fetch=True
            )
            ...
            if result and len(result) > 0:
                ...
                if template_data.get("template_content"):
                    logger.info(
                        f"Loaded {template_name} template from database for {self.administration}"
                    )
                ...
                if file_id:
                    ...
                    if provider == "s3_shared" and "/" in file_id:
                        ...
                        if content:
                            ...
                            logger.info(
                                f"Loaded {template_name} template from S3 for {self.administration}"
                            )
                            return content
                    else:
                        ...
                        if content:
                            logger.info(
                                f"Loaded {template_name} template from Google Drive for {self.administration}"
                            )
                            return content

            logger.debug(
                f"No custom {template_name} template found for {self.administration}"
            )
            return None
            ...
            if not template_content:
                ...
                if language != "en":
                    localized_template_path = os.path.join(
                        self.template_dir, f"{template_name}_{language}.{extension}"
                    )
                    ...
                    if os.path.exists(localized_template_path):
                        ...
                        logger.debug(
                            f"Loaded {template_name} template ({language}) from local file"
                        )

                # Fall back to English template
                if not template_content:
                    template_path = os.path.join(
                        self.template_dir, f"{template_name}.{extension}"
                    )
                    ...
                    if os.path.exists(template_path):
                        with open(template_path, "r", encoding="utf-8") as f:
                            template_content = f.read()
                        logger.debug(
                            f"Loaded {template_name} template (en) from local file"
                        )
                    else:
