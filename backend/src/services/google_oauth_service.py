...
def test_google_drive_connectivity(
    ...
    try:
        ...
        logger.info(
            f"Testing credentials with keys: {list(credentials.keys()) if isinstance(credentials, dict) else 'not a dict'}"
        )
        ...
        if isinstance(credentials, dict):
            if "type" in credentials and credentials.get("type") == "service_account":
                        ...
                        if tenant and credential_service:
                            try:
                                ...
                                credential_service.store_credential(
                                    tenant, "google_drive_token", refreshed_token
                                )
                                logger.info(
                ...
                logger.warning(
                    f"Unknown credential format. Keys: {list(credentials.keys())}"
                )
                return {
                    "success": False,
        ...
        _results = service.files().list(pageSize=1, fields="files(id, name)").execute()
...
def exchange_google_code_for_tokens(code, client_id, client_secret) -> dict | None:
    ...
    try:
        ...
        redirect_uri = os.getenv(
            "GOOGLE_REDIRECT_URI",
            "http://localhost:5000/api/tenant-admin/credentials/oauth/callback",
        )
        ...
        logger.info(
            f"Built complete token structure with keys: {list(complete_token.keys())}"
        )
