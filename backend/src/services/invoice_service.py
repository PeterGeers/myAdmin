...
class InvoiceService:
    ...
    def check_early_duplicates(
        ...
        try:
            ...
            params = (
                (filename, folder_name, tenant) if tenant else (filename, folder_name)
            )
            if not tenant:
    ...
    def upload_to_drive(
        ...
        if provider == "s3_shared":
            ...
            try:
                ...
                print(
                    f"Initializing Google Drive service for tenant: {tenant}",
                    flush=True,
                )
                ...
                for folder in drive_folders:
                    if folder["name"] == folder_name:
                        folder_id = folder["id"]
                        print(
                            f"Found folder: {folder_name} (ID: {folder_id})", flush=True
                        )
                        break
                ...
                if folder_id:
                    ...
                    if existing_file["exists"]:
                        print(
                            f"File {filename} already exists in Google Drive",
                            flush=True,
                        )
                        drive_result = existing_file["file"]
                    else:
                        print(
                            f"Uploading new file to Google Drive folder: {folder_name} (ID: {folder_id})",
                            flush=True,
                        )
                        drive_result = drive_service.upload_file(
                            temp_path, filename, folder_id
                        )
                    ...
                    print(
                        f"Folder '{folder_name}' not found in Google Drive folders: {[f['name'] for f in drive_folders]}",
                        flush=True,
                    )
    ...
    def process_invoice_file(
        ...
        last_transactions = self.transaction_logic.get_last_transactions(
            folder_name, tenant
        )
        ...
        if isinstance(last_transactions, dict) and last_transactions.get("error"):
            print(
                f"Transaction template error: {last_transactions['message']}",
                flush=True,
            )
            print(
                f"Building default transaction records for new vendor: {folder_name}",
                flush=True,
            )
            ...
            if transactions:
                ...
                if (
                    len(transactions) > 1
                    and "VAT" in (transactions[1].get("description") or "").upper()
                ):
            ...
            print(
                f"Created {len(default_transactions)} default transaction records for manual entry",
                flush=True,
            )
        ...
        if last_transactions:
            print(
                f"Found {len(last_transactions)} previous transactions for reference",
                flush=True,
            )
            ...
            if transactions:
                ...
                vendor_data = {
                    ...
                    "total_amount": first_tx.get("amount", 0),
                    "description": first_tx.get("description", ""),
                    "vat_amount": first_tx.get(
                        "vat_amount", 0
                    ),  # Always include vat_amount from parsed data
                }
            ...
            new_data = {
                ...
                "description": vendor_data.get(
                    "description",
                    f"PDF processed from {drive_result.get('url', 'unknown')}",
                ),
            ...
            print(
                f"Prepared {len(prepared_transactions)} new transaction records for approval",
                flush=True,
            )
    ...
    def move_file_to_folder(
        self, temp_path: str, filename: str, result_folder: str
    ) -> str:
        """
