...
class YearEndConfigService:
    ...
    def set_account_purpose(
        ...
        if purpose not in self.REQUIRED_PURPOSES:
            raise ValueError(
                f"Invalid purpose: {purpose}. Must be one of: {', '.join(self.REQUIRED_PURPOSES.keys())}"
            )
        ...
        if not account:
            raise ValueError(
                f"Account {account_code} not found for administration {administration}"
            )
        ...
        self.db.execute_query(
            update_query,
            [purpose, administration, account_code],
            fetch=False,
            commit=True,
        )
        return True
    ...
    def remove_account_purpose(
        ...
        self.db.execute_query(
            update_query, [administration, account_code], fetch=False, commit=True
        )
        return True
    ...
    def validate_configuration(self, administration: str) -> Dict[str, Any]:
        ...
        for purpose, count in purpose_counts.items():
            if count > 1:
                validation["valid"] = False
                validation["errors"].append(
                    f"Purpose '{purpose}' is assigned to multiple accounts"
                )
        ...
        for row in results:
            ...
            if row.get("vat_primary"):
                ...
                if isinstance(vat_primary_val, str):
                    ...
                    row["vat_primary"] = (
                        str(vat_primary_val) if vat_primary_val else None
                    )
