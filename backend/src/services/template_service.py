...
class TemplateService:
    ...
    def _get_local_default_metadata(
        ...
        template_path = os.path.join(
            self._templates_dir, *default["template"].split("/")
        )
        mappings_path = os.path.join(
            self._templates_dir, *default["field_mappings"].split("/")
        )
        ...
        logger.info(
            f"Using local default template for type '{template_type}': {template_path}"
        )
        return {
    ...
    def fetch_template_from_drive(
        ...
        try:
            ...
            logger.info(
                f"Fetching template file_id '{file_id}' for administration '{administration}'"
            )
    ...
    def _apply_conditional(
        ...
        try:
            ...
            condition_met = self._evaluate_condition(
                field_value, operator, compare_value
            )
    ...
    def _evaluate_condition(
        self, value: Any, operator: str, compare_value: Any
    ) -> bool:
        """
