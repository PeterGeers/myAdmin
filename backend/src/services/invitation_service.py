...
class InvitationService:
    ...
    def create_invitation(
        ...
        try:
            ...
            if existing:
                ...
                logger.info(
                    f"Updated existing invitation for {email} in {administration}"
                )
            else:
    ...
    def mark_invitation_sent(self, administration: str, email: str) -> bool:
        """
        ...
        try:
            ...
            """
            self.db.execute_query(
                query, (administration, email), fetch=False, commit=True
            )
    ...
    def mark_invitation_accepted(self, administration: str, email: str) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Marked invitation as accepted for {email} in {administration}"
            )
            return True
    ...
    def mark_invitation_failed(
        self, administration: str, email: str, error_message: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            logger.warning(
                f"Marked invitation as failed for {email} in {administration}: {error_message}"
            )
            return True
        ...
        try:
            if status:
                ...
                results = self.db.execute_query(
                    query, (administration, status), fetch=True
                )
            else:
                query = """
                    SELECT *
