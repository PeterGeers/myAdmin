...
class InvoiceEmailService:
    ...
    def _build_locale_body(self, tenant: str, invoice: dict, locale: str) -> str:
        ...
        if locale == "nl_NL":
            body = self._build_nl_body(
                addressee, invoice_number, formatted_total, formatted_due_date
            )
        else:
    ...
    def _build_nl_body(
        self, addressee: str, invoice_number: str, formatted_total: str, due_date: str
    ) -> str:
    ...
    def _build_body(self, tenant: str, invoice: dict, template_type: str) -> str:
    ...
    def _handle_send_error_with_fallback(
        ...
        is_verification_error = any(
            err_code in error_str for err_code in self._VERIFICATION_EXPIRED_ERRORS
        )
    ...
    def _build_signature_block(self, tenant: str, locale: str) -> str:
        ...
        html += f'<p style="margin: 0 0 4px 0; font-weight: bold;">{greeting}</p>'
        html += (
