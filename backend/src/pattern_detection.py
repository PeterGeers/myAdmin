...
def extract_company_name(description: str) -> Optional[str]:
    ...
    for word in words:
        ...
        if (
            len(set(word) & vowels) > 0  # Has at least one vowel
            and not word.isdigit()  # Not all digits
            and len(word) >= 3
        ):  # Minimum length
            meaningful_words.append(word)
...
def extract_verb_from_description(
    ...
    compound_verb = extract_compound_verb_from_description(
...
def analyze_reference_patterns(
    ...
    for tx in transactions:
        ...
        if is_compound and verb_reference:
            compound_key = (
                f"{administration}_{bank_account}_{verb_company}|{verb_reference}"
            )
            verb_patterns[compound_key] = {
                ...
                "occurrences": verb_patterns.get(compound_key, {}).get("occurrences", 0)
                + 1,
                "confidence": 1.0,
                "last_seen": date,
