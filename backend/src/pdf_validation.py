...
class PDFValidator:
    ...
    def validate_pdf_urls_with_progress(self, year=None, administration=None):
        ...
        try:
            ...
            records = cursor.fetchall()
            print(
                f"Found {len(records)} records for year {year}, administration {administration}"
            )
    ...
    def _file_exists(self, file_id):
        ...
        if not self.drive_service:
            print(
                f"Google Drive service not available, cannot check file ID: {file_id}"
            )
            return False
        try:
    ...
    def _find_file_in_folder(self, folder_id, filename):
        ...
        if not self.drive_service:
            print(
                f"Google Drive service not available, cannot search folder {folder_id}"
            )
            return None
        try:
            results = (
    ...
    def _gmail_message_exists(self, message_id):
        ...
        try:
            ...
            gmail_service = build(
                "gmail", "v1", credentials=self.drive_service.service._http.credentials
            )
