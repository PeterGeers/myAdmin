    ...
    def _get_booking_pace_multiplier(self, listing, date):
        ...
        try:
            ...
            if not result_df.empty and result_df.iloc[0]["total_records"] > 0:
                ...
                print(
                    f"Monthly pace data for {listing} (month {date.month}): 2024={year_2024:.0f}, 2023={year_2023:.0f}, records={total_records}"
                )
                ...
                if year_2023 > 0 and year_2024 > 0:
                    ...
                    print(
                        f"2024 vs 2023 trend ratio for {listing} (month {date.month}): {monthly_trend_ratio:.3f}"
                    )
                    ...
                    print(
                        f"Missing 2024 or 2023 data for {listing} (month {date.month}), using default 1.0"
                    )
                else:
                    print("No previous revenue data, using default 1.0")
            else:
                print(
                    f"No monthly revenue data found for {listing} (month {date.month}), using default 1.0"
                )
