...
class AuditLogger:
    ...
    def log_decision(
        ...
        try:
            ...
            logger.info(
                f"Audit log entry created: {decision} for {reference_number} on {transaction_date}"
            )
            return True
    ...
    def query_logs(
        ...
        try:
            ...
            logger.info(
                f"Audit log query returned {len(results) if results else 0} records"
            )
    ...
    def generate_compliance_report(
        ...
        try:
            ...
            continue_decisions = self.get_decision_count(
                start_date, end_date, "continue"
            )
            ...
            report["summary"] = {
                ...
                "continue_percentage": round(
                    (continue_decisions / total_decisions * 100)
                    if total_decisions > 0
                    else 0,
                    2,
                ),
                "cancel_percentage": round(
                    (cancel_decisions / total_decisions * 100)
                    if total_decisions > 0
                    else 0,
                    2,
                ),
    ...
    def get_user_activity_report(
        ...
        try:
            ...
            report["summary"] = {
                ...
                "percentage_of_all_decisions": round(
                    (user_decisions / total_decisions * 100)
                    if total_decisions > 0
                    else 0,
                    2,
                ),
            ...
            report["recent_decisions"] = self.query_logs(
                user_id=user_id, start_date=start_date, end_date=end_date, limit=20
            )
    ...
    def cleanup_old_logs(self, retention_days: int = 730) -> Tuple[bool, int]:
        ...
        try:
            ...
            self.db.execute_query(
                delete_query, (cutoff_date,), fetch=False, commit=True
            )

            logger.info(
                f"Cleaned up {delete_count} audit log entries older than {retention_days} days"
            )
    ...
    def get_audit_trail_for_transaction(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(results) if results else 0} audit trail entries for transaction"
            )
            return results if results else []
