...
def str_import_payout_authenticated(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        if not file.filename.lower().startswith(
            "payout_from"
        ...
        print(
            f"Payout import completed: {update_result['updated']} bookings updated",
            flush=True,
        )
...
def str_enrich_direct(
    ...
    try:
        ...
        metadata_key = (
            param_svc.get_param("str", "stripe_guesty_metadata_key", tenant=tenant)
            or "confirmationCode"
        )
