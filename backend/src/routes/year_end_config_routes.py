...
@year_end_config_bp.route(
    "/api/tenant-admin/year-end-config/accounts", methods=["POST"]
)
@cognito_required(required_permissions=["finance_write"])
@tenant_required()
def set_tenant_account_purpose(
    user_email, user_roles, tenant, user_tenants
) -> ResponseReturnValue:
    """
...
@year_end_config_bp.route(
    "/api/tenant-admin/year-end-config/available-accounts", methods=["GET"]
)
@cognito_required(required_permissions=["finance_read"])
@tenant_required()
def get_tenant_available_accounts(
    user_email, user_roles, tenant, user_tenants
) -> ResponseReturnValue:
    """
    ...
    try:
        ...
        if vw_filter and vw_filter not in ["Y", "N"]:
            return jsonify(
                {"success": False, "error": 'vw parameter must be "Y" or "N"'}
            ), 400
...
def configure_vat_netting(
    ...
    try:
        ...
        if not vat_accounts:
            return jsonify({"success": False, "error": "vat_accounts is required"}), 400

        if not primary_account:
            return jsonify(
                {"success": False, "error": "primary_account is required"}
            ), 400
        ...
        config_service.configure_vat_netting(tenant, vat_accounts, primary_account)
...
@year_end_config_bp.route(
    "/api/year-end-config/balance-sheet-accounts", methods=["GET"]
)
@cognito_required(required_permissions=["finance_read"])
@tenant_required()
