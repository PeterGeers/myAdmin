...
# Create blueprint
tenant_admin_details_bp = Blueprint(
    "tenant_admin_details", __name__, url_prefix="/api/tenant-admin/details"
)
...
def update_tenant_details(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info(
            f"Updated tenant details for {tenant} by {user_email}: {list(data.keys())}"
        )
