...
# Create blueprint
tenant_admin_config_bp = Blueprint(
    "tenant_admin_config", __name__, url_prefix="/api/tenant-admin"
)
...
def delete_config(user_email, user_roles, config_id) -> ResponseReturnValue:
    ...
    try:
        ...
        db.execute_query(query, (config_id, tenant), fetch=False, commit=True)
        ...
        logger.info(
            f"Deleted config {config_key} (id={config_id}) for tenant {tenant} by {user_email}"
        )
...
@tenant_admin_config_bp.route(
    "/api/tenant/config", methods=["GET"], endpoint="legacy_get_config"
)
...
@tenant_admin_config_bp.route(
    "/api/tenant/config", methods=["POST"], endpoint="legacy_set_config"
)
@cognito_required(required_permissions=[])
def set_tenant_config_legacy(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        if success:
            print(
                f"AUDIT: Tenant config updated: {tenant}.{config_key} by {user_email}",
                flush=True,
            )
            return jsonify(
                {
...
@tenant_admin_config_bp.route(
    "/api/tenant/config/<config_key>",
    methods=["DELETE"],
    endpoint="legacy_delete_config",
)
@cognito_required(required_permissions=[])
def delete_tenant_config_legacy(
    config_key, user_email, user_roles
) -> ResponseReturnValue:
    """
    ...
    try:
        ...
        print(
            f"AUDIT: Tenant config deleted: {tenant}.{config_key} by {user_email}",
            flush=True,
        )
