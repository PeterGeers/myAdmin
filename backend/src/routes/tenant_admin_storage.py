...
# Create blueprint
tenant_admin_storage_bp = Blueprint(
    "tenant_admin_storage", __name__, url_prefix="/api/tenant-admin/storage"
)
        ...
        if provider == "s3_shared":
            ...
            while True:
                ...
                for cp in response.get("CommonPrefixes", []):
                    ...
                    if folder_name:
                        folders.append({"id": folder_path, "name": folder_name})
            ...
            logger.info(
                f"Listed {len(folders)} S3 prefixes for tenant {tenant} by {user_email}"
            )
        else:
            ...
            logger.info(
                f"Listed {len(folders)} folders for tenant {tenant} by {user_email}"
            )
...
def get_storage_config(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info(
            f"Retrieved storage config for tenant {tenant}: {list(storage_config.keys())}"
        )
...
def update_storage_config(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        for key, value in data.items():
            ...
            db.execute_query(
                query, (tenant, config_key, value, user_email), fetch=False, commit=True
            )
...
def test_folder_access(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        if provider == "s3_shared":
            ...
            logger.info(
                f"Tested S3 access for tenant {tenant} by {user_email}: {test_result}"
            )

            return jsonify(
                {"success": True, "tenant": tenant, "test_result": test_result}
            )
        ...
        try:
            ...
            result = drive_service.upload_text_file(
                test_content, test_filename, folder_id, mime_type="text/plain"
            )
        ...
        logger.info(
            f"Tested folder {folder_id} for tenant {tenant} by {user_email}: {test_result}"
        )
...
def get_storage_usage(user_email, user_roles) -> ResponseReturnValue:
    ...
    try:
        ...
        if provider == "s3_shared":
            ...
            logger.info(
                f"Retrieved S3 storage usage for tenant {tenant} by {user_email}: {file_count} files, {total_size_bytes} bytes"
            )
        ...
        for config_key, folder_id in storage_config.items():
            ...
            try:
                ...
                total_size = sum(
                    int(f.get("size", 0)) for f in all_files if f.get("size")
                )
        ...
        logger.info(
            f"Retrieved storage usage for tenant {tenant} by {user_email}: {len(usage_stats)} folders"
        )
