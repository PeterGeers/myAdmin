...
@tenant_required()
def update_tax_rate(
    user_email, user_roles, tenant, user_tenants, rate_id
) -> ResponseReturnValue:
...
@tax_rate_admin_bp.route(
    "/api/tenant-admin/tax-rates/<int:rate_id>", methods=["DELETE"]
)
@cognito_required(required_permissions=[])
@tenant_required()
def delete_tax_rate(
    ...
    try:
        ...
        if administration == "_system_":
            db.execute_query(
                "DELETE FROM tax_rates WHERE id = %s",
                (rate_id,),
                fetch=False,
                commit=True,
            )
