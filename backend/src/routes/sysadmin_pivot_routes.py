...
def update_datasources(user_email, user_roles, tenant, user_tenants) -> ResponseReturnValue:
    ...
    try:
        ...
        enabled_sources = [
            s for s in sources if s.get('pivot_enabled', False)
        ]
        enabled_names = [s['name'] for s in enabled_sources]

        # Read previously registered sources to detect newly enabled ones
        prev_registered = ps.get_param(
            namespace='ui.pivot', key='registered_sources',
        ) or []
        ...
        ps.set_param(
            scope='system', scope_id='_system_',
            namespace='ui.pivot', key='registered_sources',
            value=enabled_names, value_type='json',
        ...
        for src in enabled_sources:
            ...
            if module:
                ps.set_param(
                    ...
                    value=module, value_type='string',
                    created_by=user_email,
                )
            if label:
                ps.set_param(
                    ...
                    value=label, value_type='string',
                    created_by=user_email,
                )

        # 4. Auto-create defaults for newly enabled sources
        newly_enabled = [
            name for name in enabled_names if name not in prev_set
        ]
        ...
        for name in disabled_names:
            ps.delete_param('system', '_system_', 'ui.pivot',
                            f'datasource_module.{name}')
            ps.delete_param('system', '_system_', 'ui.pivot',
                            f'datasource_label.{name}')

        logger.info(
            "Pivot datasources updated by %s: enabled=%s",
            user_email, enabled_names,
        )
...
def _auto_create_defaults(db, ps, source_name, created_by) -> None:
    ...
    try:
        ...
        groupable, aggregatable, type_map = derive_columns_from_schema(
            db, source_name, exclude_columns=set(), force_groupable=set(),
        )

        # Heuristic: numeric columns with category-like names → force groupable
        category_hints = {
            'year', 'jaar', 'month', 'maand', 'quarter', 'kwartaal',
            'week', 'q', 'm', 'period', 'periode',
        }
        force_groupable = [
            col for col in aggregatable
            if col.lower() in category_hints
        ]

        # Only write if no existing value (don't overwrite manual config)
        existing_exclude = ps.get_param(
            namespace='ui.pivot', key=f'exclude_columns.{source_name}',
        )
        ...
        existing_fg = ps.get_param(
            namespace='ui.pivot', key=f'force_groupable.{source_name}',
        )
        if existing_fg is None:
            ps.set_param(
                ...
                value=force_groupable, value_type='json',
                created_by=created_by,
            )

        logger.info(
            "Auto-created defaults for '%s': force_groupable=%s",
            source_name, force_groupable,
        )
