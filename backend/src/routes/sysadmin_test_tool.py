...
def process_file(user_email, user_roles):
    ...
    try:
        ...
        if not _validate_file_size(file):
            return jsonify({
                'success': False,
                'error': 'File exceeds maximum size of 20 MB'
            }), 400
        ...
        result = service.process_file_dry_run(temp_file_path, folder_name, administration)
...
def rerun_prompt(user_email, user_roles):
    ...
    try:
        ...
        if custom_prompt is None:
            return jsonify({'success': False, 'error': 'custom_prompt is required'}), 400
