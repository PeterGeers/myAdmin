...
class DuplicateChecker:
    ...
    def check_for_duplicates(
        ...
        try:
            ...
            self._validate_duplicate_check_params(
                reference_number, transaction_date, transaction_amount
            )

            logger.info(
                f"[{operation_id}] Starting duplicate check for {reference_number} on {transaction_date} amount {transaction_amount}"
            )
            ...
            logger.info(
                f"[{operation_id}] Duplicate check completed: {duplicate_count} matches found"
            )
            ...
            logger.error(
                f"[{operation_id}] Database connection error in duplicate check: {dce}"
            )
            # Log error but allow import to continue with warning (graceful degradation)
            self._log_graceful_degradation(
                operation_id, "database_connection", str(dce)
            )
            return []
            ...
            logger.error(
                f"[{operation_id}] Unexpected error in duplicate check: {error_details}"
            )

            # Check if this is a database connection issue
            if self._is_database_connection_error(e):
                self._log_graceful_degradation(
                    operation_id, "database_connection", str(e)
                )
                return []
        ...
        try:
            ...
            if decision not in ["continue", "cancel"]:
                raise ValidationError(
                    f"Invalid decision value: {decision}. Must be 'continue' or 'cancel'"
                )
            ...
            existing_id = (
                existing_transactions[0].get("id", "") if existing_transactions else ""
            )
            ...
            logger.info(
                f"[{operation_id}] Logging duplicate decision: {decision} for {reference_number}"
            )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info(
                        f"[{operation_id}] Duplicate decision logged successfully: {decision} for {reference_number} on {transaction_date}"
                    )
                    return True
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"[{operation_id}] Retry {attempt + 1}/{max_retries} for decision logging failed: {retry_error}"
                        )
                        continue
                    else:
                        raise retry_error
            ...
            logger.error(
                f"[{operation_id}] Session timeout error in decision logging: {ste}"
            )
            return False

        except DatabaseConnectionError as dce:
            logger.error(
                f"[{operation_id}] Database connection error in decision logging: {dce}"
            )
            return False
            ...
            logger.error(
                f"[{operation_id}] Error logging duplicate decision: {error_details}"
            )

            # Check if this is a missing table error (migrations not run yet)
            if self._is_missing_table_error(e):
                logger.warning(
                    f"[{operation_id}] Duplicate decision log table not found - migrations may not be complete"
                )
                return False
    ...
    def _validate_duplicate_check_params(
    ...
    def _check_session_timeout(
        ...
        try:
            ...
            logger.debug(
                f"[{operation_id}] Session timeout check passed for session: {session_id}"
            )
    ...
    def _is_database_connection_error(self, error: Exception) -> bool:
        ...
        return any(
            indicator in error_message for indicator in connection_error_indicators
        )
    ...
    def _log_graceful_degradation(
        self, operation_id: str, error_type: str, error_message: str
    ) -> None:
        """
        ...
        logger.warning(
            f"[{operation_id}] Graceful degradation triggered: {degradation_log}"
        )
    ...
    def _send_monitoring_alert(self, degradation_log: Dict) -> None:
        ...
        logger.debug(
            f"Monitoring alert prepared for: {degradation_log['operation_id']}"
        )
