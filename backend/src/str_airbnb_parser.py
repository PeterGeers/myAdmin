...
def process_airbnb_multi(
    ...
    if "Bevestigingscode" in combined.columns:
        ...
        print(
            f"Airbnb multi-import: deduplicated {before_dedup} -> {len(combined)} rows (by Bevestigingscode)"
        )
    ...
    print(
        f"Airbnb multi-import: {len(transactions)} transactions from {len(file_paths)} file(s)"
    )
    return transactions
...
def calculate_airbnb_row(
    ...
    gross_amount = (
        paid_out + amount_channel_fee
    )  # R: amountGross = paidOut + amountChannelFee

    # Use generic tax calculation function
    tax_calc = calculate_str_taxes(
        gross_amount, checkin_date, amount_channel_fee, tax_rate_service, tenant
    )
