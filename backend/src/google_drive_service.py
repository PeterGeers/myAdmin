...
class GoogleDriveService:
    ...
    def _authenticate(self):
        ...
        try:
            ...
            logger.info(
                f"Retrieving Google Drive credentials for administration: {self.administration}"
            )

            # Get OAuth credentials (credentials.json content)
            oauth_creds = credential_service.get_credential(
                self.administration, "google_drive_oauth"
            )
            ...
            token_data = credential_service.get_credential(
                self.administration, "google_drive_token"
            )
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    # Refresh the token
                    logger.info(
                        f"Refreshing expired token for administration: {self.administration}"
                    )
                    try:
                        ...
                        credential_service.store_credential(
                            self.administration, "google_drive_token", token_info
                        )
                        logger.info(
                        ...
                        logger.error(
                            "⚠️  The refresh token may have expired or been revoked."
                        )
                    ...
                    logger.warning(
                        f"⚠️  No valid token found for administration: {self.administration}"
                    )
            ...
            logger.info(
                f"Successfully authenticated Google Drive for administration: {self.administration}"
            )
            return service

        except Exception as e:
            logger.error(
                f"Failed to authenticate Google Drive for administration '{self.administration}': {e}"
            )
            raise
    ...
    def list_subfolders(self):
        ...
        try:
            ...
            while True:
                page_count += 1
                print(
                    f"📄 Fetching page {page_count} from Google Drive (pageToken: {page_token})",
                    flush=True,
                )
                ...
                print(
                    f"📦 Page {page_count} returned {len(subfolders)} folders",
                    flush=True,
                )
                ...
                if not page_token:
                    print(
                        f"✅ Pagination complete. Total folders: {len(all_subfolders)}",
                        flush=True,
                    )
                    break
            ...
            if len(folder_names) != len(set(folder_names)):
                ...
                duplicates = [
                    name for name, count in Counter(folder_names).items() if count > 1
                ]
                print(
                    f"WARNING: Found duplicate folder names in Facturen folder: {duplicates}"
                )
                print(
                    f"Folder details: {[f for f in all_subfolders if f['name'] in duplicates]}"
                )
