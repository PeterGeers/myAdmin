...
class PDFProcessor:
    ...
    def _format_vendor_transactions(self, vendor_data, file_data):
        ...
        folder_name = (
            file_data["folder"].split("/")[-1]
            if "/" in file_data["folder"]
            else file_data["folder"]
        )
        reference_number = folder_name.replace("testFacturen/", "").replace(
            "Facturen/", ""
        )
        ...
        try:
            ...
            main_credit = (
                last_transactions[0]["Credit"] if last_transactions else "1300"
            )
            vat_debet = (
                last_transactions[1]["Debet"] if len(last_transactions) > 1 else "2010"
            )
            vat_credit = (
                last_transactions[1]["Credit"]
        ...
        try:
            duplicate_info = self._check_for_duplicates(
                transactions, reference_number, file_data
            )
