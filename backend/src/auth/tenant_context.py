...
def get_user_tenants(jwt_token: str) -> List[str]:
    ...
    try:
        ...
        print(
            f"[Backend] Raw tenants from JWT: {tenants} (type: {type(tenants).__name__})",
            flush=True,
        )
        ...
        if isinstance(tenants, str):
            try:
                ...
                print(
                    f"[Backend] JSON parse failed: {e}, treating as single tenant",
                    flush=True,
                )
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not is_authorized:
                print(
                    f"❌ Tenant access denied for {f.__name__}: {error_response}",
                    flush=True,
                )
...
def set_tenant_config(
    ...
    try:
        ...
        db.execute_query(
            query,
            [tenant, config_key, config_value, is_secret, created_by],
            fetch=False,
            commit=True,
        )
        return True
