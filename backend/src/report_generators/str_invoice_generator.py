...
def generate_table_rows(invoice_data: dict[str, Any], language: str = "nl") -> str:
    ...
    try:
        ...
        logger.info(
            f"Generated {len(rows)} table rows for STR invoice in language '{language}'"
        )
...
def prepare_invoice_data(
    booking_data: dict[str, Any], custom_billing: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Prepared invoice data for reservation {invoice_data['reservationCode']}"
        )
