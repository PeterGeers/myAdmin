...
def generate_toeristenbelasting_report(
    ...
    try:
        ...
        rental_stats = _calculate_rental_statistics(
            bnb_data=bnb_data, aantal_kamers=config["aantal_kamers"]
        )
...
def _calculate_rental_statistics(
    ...
    totaal_verhuurde_nachten = sum(b.get("nights", 0) for b in bnb_data["all_bookings"])

    cancelled_nachten = sum(b.get("nights", 0) for b in bnb_data["cancelled_bookings"])
...
def _get_financial_data(
    ...
    saldo_toeristenbelasting = abs(
        _get_tourist_tax_from_account(
...
def _calculate_taxable_revenue(financial_data: Dict[str, float]) -> Dict[str, float]:
    ...
    belastbare_omzet_logies = (
        financial_data["ontvangsten_excl_btw_excl_toeristenbelasting"] - totaal_2_3_4
    )
...
def _prepare_template_data(
    ...
    return {
        ...
        "verhuurde_kamers_inwoners": str(
            int(rental_stats["verhuurde_kamers_inwoners"])
        ),
        "totaal_belastbare_nachten": str(
            int(rental_stats["totaal_belastbare_nachten"])
        ),
        ...
        "saldo_toeristenbelasting": format_currency(
            financial_data["saldo_toeristenbelasting"], currency="EUR"
        ),
        ...
        "no_show_omzet": format_amount(financial_data["no_show_omzet"]),
        "totaal_2_3_4": format_amount(taxable_revenue["totaal_2_3_4"]),
