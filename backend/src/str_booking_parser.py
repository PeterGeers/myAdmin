...
def process_booking(file_path: str, tax_rate_service=None,
    ...
    try:
        ...
        for _, row in df.iterrows():
            booking = calculate_booking_row(row, df.columns, source_file,
...
def process_booking_multi(file_paths: List[str], tax_rate_service=None,
                          tenant: str = None) -> List[Dict]:
    """
    ...
    for _, row in combined.iterrows():
        booking = calculate_booking_row(row, combined.columns, source_file,
                                        tax_rate_service, tenant)
        if booking is not None:
            transactions.append(booking)
...
def calculate_booking_row(row, df_columns, source_file: str,
    ...
    tax_calc = calculate_str_taxes(price, checkin_date, amount_channel_fee,
                                   tax_rate_service, tenant)
...
def process_booking_payout(file_path: str, tax_rate_service=None,
