...
def setup_test_database():
    ...
    # Connect via DatabaseManager to the finance database initially
    db = DatabaseManager()
    # Override to connect without a specific database for DDL operations
    db.config['database'] = 'finance'
                ...
                while remaining_views and attempt < max_attempts:
                    ...
                    for view in remaining_views:
                        try:
                            ...
                            # Get view definition using dialect helper
                            cursor.execute(dialect.get_view_definition(f"finance.{view}"))
